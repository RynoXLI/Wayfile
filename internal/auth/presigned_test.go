...
func TestGenerateAndVerifyToken(t *testing.T) {
	...
	docID := "abc-123-def-456"

	// Generate token
	token := signer.GenerateToken(namespace, docID, 1*time.Hour)
	...
	if gotNs != namespace {
		t.Errorf("expected namespace %q, got %q", namespace, gotNs)
	}
...
func TestTokenExpiration(t *testing.T) {
	...
	token := signer.GenerateToken(namespace, docID, -1*time.Second)
...
func TestDifferentSecrets(t *testing.T) {
	...
	token := signer1.GenerateToken(namespace, docID, 1*time.Hour)
